...
struct object* reem_get_last() {
	struct object * last_obj1 = first_object;
	while (last_obj1->next != NULL) {
		last_obj1 = last_obj1->next;	
	}
	return last_obj1;
}
int reem_get_number_of_lines(char* file_name123) {
	int counter_line = 0;
	FILE * fff;
	int file_name_len = strlen(file_name123);
	fff = fopen(file_name123, "r");
	char buf [1024];
	while (fgets(buf, sizeof(buf), fff) != NULL)
{
	counter_line++;
}
fclose(fff);
...
load_file(FILE *fp, char *name, int calltype)
{
	...
	char ffff[1024];
	double start_tt;
	struct object *first_firstobject0 = NULL;
	struct object *first_firstobject1 = NULL;
	struct object *first_firstobject2 = NULL;
	struct object *first_firstobject3 = NULL;
	struct object *last_firstobject0 = NULL;
	struct object *last_firstobject1 = NULL;
	struct object *last_firstobject2 = NULL;
	struct object *last_firstobject3 = NULL;
	int num_of_threads;
	int i, jj ,j;
    int start, left;
    int more;
    int stop = FALSE;
	int max_contours = reem_get_number_of_lines(name);
	data_reading = (char**) malloc(sizeof(char*)*max_contours);
	for (i = 0; i < max_contours; i++) {
		data_reading[i] = (char*)malloc(sizeof(char)*1024);
	}
    ...
    interactive = FALSE;
	start_tt = omp_get_wtime();
	for (i = 0; i < max_contours; i++) {
		fgets(&(data_reading[i][0]), 1024, fp);
	}
	jj = 1;
#pragma omp parallel
{
	...
	printf("Read file took: %10e\n", omp_get_wtime() - start_tt);
i = 0;
	#pragma omp parallel for firstprivate(jj, start, left, more, len)
	for(i =0 ; i < max_contours; i++) {
		char gp_input_line_holder[1024];
		left = gp_input_line_len;
		start = 0;
//		#pragma omp simd
//		for(j = 0; j < 1024; j++)
//		{
//			gp_input_line_holder[j] = data_reading[i][j];
//		}
		gp_input_line = &data_reading[i][0];
	...
	#pragma omp parallel 
{
		if (omp_get_thread_num()  == 0) {
			last_firstobject0 = reem_get_last();
			first_firstobject0 = first_object;
		}
		if (omp_get_thread_num()  == 1) {
			last_firstobject1 = reem_get_last();
			first_firstobject1 = first_object;
		}
		if (omp_get_thread_num()  == 2) {
			last_firstobject2 = reem_get_last();
			first_firstobject2 = first_object;
		}
		if (omp_get_thread_num()  == 3) {
			last_firstobject3 = reem_get_last();
			first_firstobject3 = first_object;
		}
}
#pragma omp parallel
{
	if(omp_get_thread_num() == 0) {
		...
		if (num_of_threads == 4) {
			last_firstobject2->next = first_firstobject3;
			num_of_threads --;
		}
		if (num_of_threads == 3) {
			last_firstobject1->next = first_firstobject2;
			num_of_threads --;
		}
		if (num_of_threads == 2) {
			last_firstobject0->next = first_firstobject1;
			num_of_threads --;
		}
	}
}
